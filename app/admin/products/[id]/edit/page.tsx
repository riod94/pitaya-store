...
export default function EditProductPage() {
	...
	useEffect(() => {
		...
		Promise.all([
			...
			fetch(`/api/admin/products/${productId}/variants`).then((res) =>
				res.json()
			),
		])
			.then(([productData, variantsData]) => {
				...
				setPrice(prod.sellingPrice?.toString() || "");
				setDiscountPercent("");
				setDiscountPrice("");
				...
				setPreOrder(false);
				setShippingCourier(false);
				setShippingInsurance("optional");
				...
				if (variantsData.data && variantsData.data.length > 0) {
					...
					const loadedVariants = variantsData.data.map((variant: any) => ({
						id: variant.id,
						name: variant.name,
	...
	const handleVariants = async (productId: string, variants: any[], productData: any) => {
		// Get existing variants from database
		const existingVariantsResponse = await fetch(`/api/admin/products/${productId}/variants`);
		...
		// Track which variants need to be deleted
		const submittedVariantIds = new Set(variants.map(v => v.id));
		const variantsToDelete = existingVariants.filter(
			(v: any) => !submittedVariantIds.has(v.id)
		);
		...
		for (const variant of variants) {
			...
			// If variant has an ID and exists in database, update it
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (variantEnabled && variants.length > 0) {
				await handleVariants(productId as string, variants, productData);
			} else if (variantEnabled && variants.length === 0) {
				// If variants are disabled but were previously enabled, delete all existing variants
	...
	return (
		<div className="container fluid rounded-lg space-y-6">
			<Link href="/admin/products" className="flex gap-4">
				<ArrowLeft size={28} strokeWidth={2} />
				<p className="text-2xl font-bold">Edit Produk</p>
			</Link>

			{/* Informasi Dasar */}
			<section className="bg-white p-6 rounded-lg shadow space-y-4">
				<h2 className="text-lg font-semibold border-l-4 border-pink-500 pl-2">
					Informasi Dasar
				</h2>
				<div className="flex flex-col md:flex-row gap-6">
					...
					<div className="flex-1 space-y-4">
						...
						<div>
							...
							<Input
								placeholder="Pilih atau ketik kategori produkmu"
								value={category}
				...
				<div className="space-y-4">
					...
					<div className="flex items-center justify-between">
						...
						<Switch
							...
							onCheckedChange={setVariantEnabled}
						/>
					</div>
					...
					{!variantEnabled && (
						<>
							<div>
								...
								<Input
									placeholder="Rp Masukkan harga produkmu"
									value={price}
							...
							<div className="flex gap-2">
								...
								<div className="flex-1">
									...
									<Input
										placeholder="Rp Masukkan harga diskon"
										value={discountPrice}
							...
							<div>
								<Label>SKU</Label>
								<Input
									placeholder="Masukkan SKU Produkmu"
									value={sku}
			...
			<section className="bg-white p-6 rounded-lg shadow space-y-4">
				...
				<div className="space-y-4">
					...
					<div className="flex items-center justify-between">
						...
						<Switch
							checked={shippingCourier}
							onCheckedChange={setShippingCourier}
						/>
					</div>
					<div>
			...
			<div className="flex justify-end space-x-4">
				...
				>
					...
					{loading || saving ? "Menyimpan..." : "Simpan"}
				</Button>
			</div>
		</div>
	);
