...
const products = [
	{
		...
		description:
			"Premium quality pistachios with shells, perfect for snacking.",
		badge: "Best Seller",
	},
	{
		id: 2,
	...
	},
	{
		...
		image: "/placeholder.svg?height=300&width=300",
		description:
			"Dried cranberries, a sweet and tangy addition to your diet.",
	},
	{
		id: 7,
		name: "Chia Seed",
...
// Komponen reusable untuk selector varian
				...
				{weights.slice(0, 2).map((weight) => (
					<div key={weight} className="flex-1">
						<button
							...
							className={`w-full text-center p-2 border rounded-lg cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-pink-400 ${
								selected === weight
									? "border-pink-500 bg-pink-50"
									: "border-gray-200 hover:border-pink-500 hover:bg-pink-50"
							}`}
							onClick={() => onSelect(selected === weight ? "" : weight)}
						>
...
export default function ProductSection() {
	// State untuk menyimpan varian yang dipilih per produk
			...
			<div className="container mx-auto px-4">
				<div className="text-center mb-12">
					...
					<p className="text-gray-600 max-w-2xl mx-auto">
						Explore our selection of high-quality nuts and seeds, sourced
						from the best suppliers and carefully processed to preserve
					...
					{products.map((product) => (
						...
						>
							...
							<div className="p-5">
								...
								{/* Button */}
								<Button
									className="w-full bg-gradient-to-r from-pink-500 to-teal-500 hover:from-pink-600 hover:to-teal-600 text-white rounded-xl flex items-center justify-center gap-2"
									onClick={() => {
										const selected = selectedVariants[product.id];
										if (!selected) return;
										const harga =
											product.prices[
												selected as keyof typeof product.prices
											];
										const pesan = encodeURIComponent(
											`Halo, saya ingin membeli produk ${product.name} varian ${selected} (Rp ${harga}) dari Pitaya Store.`
										);
										window.open(
											`https://wa.me/6282277563627?text=${pesan}`,
											"_blank"
										);
									}}
				...
				<div className="mt-12 text-center">
					<Button
						variant="outline"
						className="border-teal-500 text-teal-500 hover:bg-teal-50 px-6 py-2 rounded-full"
					>
